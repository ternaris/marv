...
The ``raw_messages`` node pushes raw ROS messages, which have to be deserialized using the correct message class.

.. note::

   Work is in progress to use the message type definition stored in the bag file for the connection that produced the message. The current code will fail if the message definition during runtime differs from that during recording time.
